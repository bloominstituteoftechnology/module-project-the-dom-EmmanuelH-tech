function moduleProject1() {
  ...
  const firstWid = document.querySelector('.quoteoftheday')
  firstWid.appendChild(quote)
  const authorDate = document.createElement('div')
  const { author, date } = randomQuote
  authorDate.textContent = `${author} in ${date || 'an unknown date'}`
  firstWid.appendChild(authorDate)
...
document.querySelector('.corporatespeak').appendChild(paragraph)
  ...
  let sentence = `${randomPerson.fname} ${randomPerson.lname} was born in ${year} and `
   ...
   if (!randomPerson.friends.length) {
    ...
    let isNextLastIdx = idx === randomPerson.friends.length - 2
    if (isLastIdx) {
      sentence += `${fullName}.`
    } else if (isNextLastIdx) {
